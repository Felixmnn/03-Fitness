...
export const handleLogin = async () => {
    try {
      // Starte den OAuth Login mit Google
      const response = await account.createOAuth2Session('google', 'http://localhost:8081/sign-in');
      console.log("Response:", response);
  
      if (response.href) {
        // Navigiere zur URL im externen Browser (nicht in einem eingebetteten Webview)
        Linking.openURL(response.href);  // Öffnet die URL im externen Browser
      } else {
        console.error("Keine Redirect-URL erhalten");
      }
    } catch (error) {
  ...
  export  const handleUser = async () => {
    try {
      ...
      const session = await account.createOAuth2Session(user);
            return session;
    } catch (error) {
  ...
  export const handleOAuthLogin = async () => {
    try {
      // OAuth-Anmeldung einleiten; beachte hier die korrekte Redirect URI
      const response = await account.createOAuth2Session('google', 'http://localhost:8081/sign-in', 'http://localhost:8081/failure');
      if (response.href) {
        // Navigiere zur URL im externen Browser (nicht in einem eingebetteten Webview)
        Linking.openURL(response.href);  // Öffnet die URL im externen Browser
      } else {
      ...
      console.error('Fehler beim Starten des OAuth-Prozesses:', error);
    }
  };
...
export const syncWorkouts = async () => {
    //Gets all Keys in Clout 
    //Gets alle Keys in Async
    //Compares them --> Syncs them
}
